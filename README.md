...
- Adjacent duplicate mouse lines suppressed (same process, title, event code, and details)
- 托盘应用，支持暂停/恢复与退出控制
- 事件采集管线，带有有界缓冲
- 支持按进程名和窗口标题进行过滤
- 可选键盘文本采集
- 按天输出本地 Markdown 日志
- 相邻且完全相同的鼠标事件只记一条，减少刷屏（同进程、标题、事件码与 details）
...
[![Star History Chart](https://api.star-history.com/svg?repos=banlian/winrecorder&type=Date)](https://www.star-history.com/?repos=banlian%2Fwinrecorder&type=Date)
...
### Test (self-test executable)

The test project is a small console runner (not a VSTest suite).
...
### Why are repeated mouse clicks not logged every time?
### 为什么连续点击不会每次都记一条？

Consecutive **mouse** events with the same process name, window title, event code, and `details` are **deduplicated**: only the first in each run is written, until one of those fields changes. (Keyboard and other event types are still logged, but they do not reset this mouse “same as last” state.)
在**进程名、窗口标题、事件码、details** 都相同的情况下，连续出现的**鼠标**事件只记第一条，直到其中任一字段与上一次已记录的鼠标事件不同。其它类型事件（如键盘）照常记录，但**不会**单独用来“打断”上述鼠标去重状态。
